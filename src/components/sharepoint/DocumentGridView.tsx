        ...
        clients.map((client) => (
          ...
          >
            ...
            <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500">
              ...
              <span className="font-mono">{new Date(client.updated_at).toLocaleDateString('vi-VN')}</span>
            </div>
          </div>
        ))}
        ...
        subFolders.map((sf) => (
          ...
          >
            ...
            <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500">
              ...
              <span className="font-mono">Fica Hub</span>
            </div>
          </div>
        ))}

      {/* Document Files in Grid */}
      {!isFolderView &&
        !isSubFolderView &&
        files.map((file) => (
          <div
            key={file.id}
            className="group bg-white border border-slate-200 rounded-xl p-4 shadow-xs hover:shadow-md hover:border-blue-400 transition-all flex flex-col justify-between"
          >
            <div>
              <div className="flex items-start justify-between mb-3">
